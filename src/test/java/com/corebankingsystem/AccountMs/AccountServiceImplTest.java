...
public class AccountServiceImplTest {
    ...
    public void testDeleteAccount_Success() {
        when(accountRepository.findById(1L)).thenReturn(Optional.of(accountsArray[0]));
        ResponseEntity<?> response = accountServiceImpl.deleteAccount(1L);
        assertEquals(200, response.getStatusCodeValue());
    ...
    public void testDeleteAccount_AccountNotFound() {
        ...
        ResponseEntity<?> response = accountServiceImpl.deleteAccount(999L);
...
// Test for getCustomerId method
    ...
    when(accountRepository.findByCustomerId(1L)).thenReturn(Optional.of(Arrays.asList(accountsArray[0], accountsArray[1])));

        Optional<List<Account>> result = accountServiceImpl.getCustomerId(1L);

        assertTrue(result.isPresent());
        assertEquals(2, result.get().size());
        assertEquals("123456", result.get().get(0).getAccountNumber());
    }

    @Test
    void testGetCustomerId_NoAccountsFound() {
        when(accountRepository.findByCustomerId(99L)).thenReturn(Optional.empty());

        Optional<List<Account>> result = accountServiceImpl.getCustomerId(99L);

        assertFalse(result.isPresent());
    }

    // Test for generateAccountNumber method
    @Test
    void testGenerateAccountNumber() {
        String generatedAccountNumber = accountServiceImpl.generateAccountNumber();
        assertNotNull(generatedAccountNumber);
        assertEquals(6, generatedAccountNumber.length());
    }
    ...
    void testGetAccountByNumber_NotFound() {
        ...
        Optional<Account> result = accountServiceImpl.getAccountByNumber("999999");
    ...
    @Test
    void testTDeposit_AccountNotFound() {
        when(accountRepository.findByAccountNumber("999999")).thenReturn(Optional.empty());

        ResponseEntity<Object> result = accountServiceImpl.tDeposit("999999", 500.0);
    ...
    @Test
    void testTWithdrawal_AccountNotFound() {
        when(accountRepository.findByAccountNumber("999999")).thenReturn(Optional.empty());

        ResponseEntity<?> result = accountServiceImpl.tWithdrawal("999999", 500.0);
